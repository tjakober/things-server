...
function M.sendInfo(oResp)
    ...
    local lfstime, lfsmodules, lfslist
    local LFS = node.LFS
    if (LFS) then
        local tm = rtctime.epoch2cal(node.LFS.time)
        lfstime = string.format("%04d/%02d/%02d %02d:%02d:%02d", tm["year"], tm["mon"], tm["day"], tm["hour"], tm["min"], tm["sec"])
        lfsmodules = ''
        lfslist = LFS.list()
        for i, t in ipairs(lfslist) do 
            lfsmodules = lfsmodules .. t .. ', ' 
        end
    ...
    local oInfo = {
        cmd = 'nodeInfo',
        from = transmit.oResp.from,
        to = transmit.oResp.to,
